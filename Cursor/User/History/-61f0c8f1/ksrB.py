...
class ScheduleInput(ScheduleBase):
    def calculate_timetable(self, service) -> ScheduleOutput:
        ...
        days_delta = first_schedule.day_number  # Use day number directly instead of week calculation
        schedule_date = self.start_date + timedelta(days=days_delta)
        ...
        # Use the minimum between capacity and number of recipients
        mails_to_send = min(len(remaining_recipients), daily_capacity)
        
        for index in range(mails_to_send):
            schedule_list = []
            ...
            for week_schedule in self.schedule:
                ...
                date = self.start_date + timedelta(days=days_delta)
            ...
            detail = RecipientDetail(
                recipient_id=remaining_recipients[index],
