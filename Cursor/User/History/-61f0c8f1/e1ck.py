...
class ScheduleInput(ScheduleBase):
    def calculate_timetable(self, service) -> ScheduleOutput:
        ...
        while remaining_recipients:
            ...
            # For the current batch's first scheduled day, get its capacity
            first_schedule = self.schedule[0]
            days_delta = (first_schedule.week - 1) * 7 + first_schedule.day_number
            schedule_date = self.start_date + timedelta(days=days_delta + (7 * (current_batch - 1)))
            ...
            # Use the minimum between capacity and remaining recipients
            mails_to_send = min(len(remaining_recipients), daily_capacity)
            
            if mails_to_send == 0:
                break  # No more capacity available
            
            batch_recipients = remaining_recipients[:mails_to_send]
            remaining_recipients = remaining_recipients[mails_to_send:]
            ...
            for index, recipient_id in enumerate(batch_recipients):
                ...
                gap_inbetween = int(time_diff_in_minutes / len(batch_recipients))
                ...
                for week_schedule in self.schedule:
                    ...
                    date = schedule_date + timedelta(days=days_delta)
                    schedule_time = datetime.combine(date, week_schedule.start_time) + timedelta(minutes=send_pace * index)
