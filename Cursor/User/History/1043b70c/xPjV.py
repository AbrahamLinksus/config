...
from app.contracts import ScheduleInput, RecipientSchedule, SchedulingStrategy
...
def test_scheduler_for_low_capacity():
    ...
    input = ScheduleInput.model_validate(data)
    output = Scheduler(service).schedule(input, strategy=SchedulingStrategy.SINGLE_BATCH)
    ...
    assert recipient_ids == [111, 222]

def test_scheduler_for_batch2_on_the_next_start():
    service = MagicMock(spec=CampaignService)
    low_capacities = generate_dates(datetime.now().date(), [1 for _ in range(7*4)])
    service.calculate_capacity.return_value = low_capacities
    start_date = low_capacities[0].send_date
    ...
    input = ScheduleInput.model_validate(data)
    output = Scheduler(service).schedule(input, strategy=SchedulingStrategy.ALL_BATCHES)

    assert len(output.timetable) == 4
    batch_numbers = {r.batch_number for r in output.timetable}
    assert batch_numbers == {1, 2, 3, 4}

def test_scheduler_with_max_batches():
    service = MagicMock(spec=CampaignService)
    low_capacities = generate_dates(datetime.now().date(), [1 for _ in range(7*4)])
    service.calculate_capacity.return_value = low_capacities
    start_date = low_capacities[0].send_date
    ...
    input = ScheduleInput.model_validate(data)
    output = Scheduler(service).schedule(input, strategy=SchedulingStrategy.MAX_BATCHES, max_batches=2)
