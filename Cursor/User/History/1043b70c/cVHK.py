...
def test_scheduler_with_one_email_short_window():
    ...
    # Test case 1: Large window (8 hours = 480 minutes) for 3 people
    # Should use 5-minute gaps since 480/3 > 5
    data = {
      "campaign_id": 1,
      "sending_account_id": 100,
      "sending_limit": 20,
      "schedule": [{
          ...
          "week": 1,
          "start_time": "09:00:00",
          "end_time": "17:00:00",  # 8-hour window
          "timezone": "UTC",
          "send_as_reply": True
      }],
      "start_date": start_date,
    ...
    # Test case 2: Short window (10 minutes) for 3 people
    # Should use 3.33-minute gaps (rounded to minutes) since 10/3 < 5
    data["schedule"][0].update({
        "start_time": "09:00:00",
        "end_time": "09:10:00"  # 10-minute window
    })
    input = ScheduleInput.model_validate(data)
    output = Scheduler(service).schedule(input)

    assert len(output.timetable) == 3
    # Verify compressed gaps (approximately 3 minutes each)
    times = [r.schedule[0].send_time.time() for r in output.timetable]
    assert times == [
        time(9, 0, 0),
        time(9, 3, 0),
