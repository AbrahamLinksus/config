...
from app.contracts import ScheduleInput, RecipientSchedule, Date_validator
...
def test_scheduler_with_older_date():
    service = MagicMock(spec=CampaignService)
    service.calculate_capacity.return_value = LARGE_CAPACITIES
    start_date = Date_validator(LARGE_CAPACITIES[0].send_date)
    data = {
      "campaign_id": 1,
      "sending_account_id": 100,
      "sending_limit": 20 ,
...
def test_scheduler_for_a_passed_time():
    ...
    assert(input.schedule[0].start_time) > datetime.now().time(), "The given start time has passed"

def test_schedule_for_short_windows():
    service = MagicMock(spec=CampaignService)
    service.calculate_capacity.return_value = LARGE_CAPACITIES
    start_date = Date_validator(input_date=LARGE_CAPACITIES[0].send_date)
    data = {
      "campaign_id": 1,
      "sending_account_id": 100,
      "sending_limit": 20 ,
