...
def test_scheduler_with_three_recipients_short_window():
    service = MagicMock(spec=CampaignService)
    service.calculate_capacity.return_value = LARGE_CAPACITIES
    start_date = LARGE_CAPACITIES[0].send_date
    data = {
    ...
    assert timetable[0].schedule[0].send_time == datetime.combine(start_date + timedelta(days=2), time(9, 0, 0))
    assert timetable[1].schedule[0].send_time == datetime.combine(start_date + timedelta(days=2), time(9, 3, 0))
    assert timetable[2].schedule[0].send_time == datetime.combine(start_date + timedelta(days=2), time(9, 6, 0))
