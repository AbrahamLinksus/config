...
class Scheduler:
    ...
    def schedule(self, input: ScheduleInput) -> ScheduleOutput:
        ...
        for index, recipient_id in enumerate(input.recipients):
            ...
            gap_inbetween = int(datetime.combine(datetime_dummy,(input.schedule[0].end_time)) - datetime.combine(datetime_dummy,(input.schedule[0].start_time).total_seconds()/60)) 
